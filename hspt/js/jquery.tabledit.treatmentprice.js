    ...
    $.fn.Tabledit = function(options) {
        ...
        if (settings.deleteButton) {
            ...
            $table.on('click', 'button.tabledit-confirm-button', function(event) {
                ...
                $.post({
                    url: 'api/deleteDiseasePrice.php',
                    data: JSON.stringify({
                        // User
                        // account: "admin",
                        token: token,
        ...
        if (settings.editButton) {
            ...
            $table.on('click', 'button.tabledit-edit-button', function(event) {
                ...
                for(var old_index=0;old_index<treatmentPriceData.data.diseasePrice.length;old_index++){  
                    if(treatmentPriceData.data.diseasePrice[old_index].ID == flag){
                        old_diseaseID =  treatmentPriceData.data.diseasePrice[old_index].diseaseID;    
                        old_departmentID =  treatmentPriceData.data.diseasePrice[old_index].departmentID;
                        old_treatmentTypeID =  treatmentPriceData.data.diseasePrice[old_index].treatmentTypeID;
                        old_price  = treatmentPriceData.data.diseasePrice[old_index].price;
                    }
                }
                $(this).parent().parent().parent().parent().find("select[name='disease'] option[value="+ old_diseaseID +"]").prop("selected",true);      
                $(this).parent().parent().parent().parent().find("select[name='department'] option[value="+ old_departmentID +"]").prop("selected",true);   
                $(this).parent().parent().parent().parent().find("select[name='type'] option[value="+ old_treatmentTypeID +"]").prop("selected",true);                                                                                        
            });
            ...
            $table.on('click', 'button.tabledit-save-button', function(event) {
                ...
                var new_name = $(this).parent().parent().parent().find("input[name='treatmentName']").val();
                var new_treatmentTypeID = $(this).parent().parent().parent().find('option:selected').val();
                var td_index = $(this).parent().parent().parent().prop('id');
                var diseaseID = $(this).parent().parent().parent().find("select[name='disease']").find('option:selected').val();  
                var departmentID = $(this).parent().parent().parent().find("select[name='department']").find('option:selected').val(); 
                var treatmentTypeID = $(this).parent().parent().parent().find("select[name='type']").find('option:selected').val(); 
                ...
                $.post({
                    ...
                    data: JSON.stringify({
                        ...
                        price: price,
                        old_diseaseID: old_diseaseID,
                        old_departmentID: old_departmentID,
                        old_treatmentTypeID: old_treatmentTypeID,
                        old_price: old_price
                    }),      
                    success: function (jsonData) { // Callback function when successful
                        viewDeletemessage(jsonData); // Call VIEW function 

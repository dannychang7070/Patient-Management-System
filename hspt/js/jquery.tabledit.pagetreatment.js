    ...
    $.fn.Tabledit = function(options) {
        ...
        if (settings.deleteButton) {
            ...
            $table.on('click', 'button.tabledit-confirm-button', function(event) {
                ...
                $.post({
                    url: 'api/deleteTreatment.php',
                    data: JSON.stringify({
                        // User
                        // account: "admin",
                        token: token,
                        treatmentID: td_index           
                    }),      
        ...
        if (settings.editButton) {
            ...
            $table.on('click', 'button.tabledit-edit-button', function(event) {
                ...
                for(var old_index=0;old_index<treatmentData.data.treatment.length;old_index++){  
                    if(treatmentData.data.treatment[old_index].ID == flag){
                        old_treatmentTypeID  =  treatmentData.data.treatment[old_index].treatmentTypeID;
                    }
                }                    
                old_name = $(this).parent().parent().parent().parent().find("input[name='treatmentName']").val();    
                $(this).parent().parent().parent().parent().find("select[name='treatmentType'] option[value="+ old_treatmentTypeID +"]").prop("selected",true);                  
            });
            ...
            $table.on('click', 'button.tabledit-save-button', function(event) {
                ...
                $.post({
                    ...
                    data: JSON.stringify({
                        ...
                        treatmentID: td_index, 
                        name: new_name, 
                        treatmentTypeID: new_treatmentTypeID,
                        old_name: old_name,

    ...
    $.fn.Tabledit = function(options) {
            ...
            columns: {
                identifier: function() {
                    ...
                    $td.each(function() {
                        ...
                        $(this).parent('tr').attr('class',"dataSect");
                    });
                },
                editable: function() {
                    for (var i = 0; i < settings.columns.editable.length; i++) {
                        ...
                        $td.each(function() {
                            ...
                            if (i==0){
                                var name = merchandiseData.data.merchandise[count].name;
                                span = '<a href="merchandise.html?account=admin&name='+encodeURIComponent(name)+'">' + span + '</a>';
                            }

                            if (i==1){
                                var name = merchandiseData.data.merchandise[count].name;
                                var size = merchandiseData.data.merchandise[count].size;
                                span = '<a href="merchandise.html?account=admin&name='+encodeURIComponent(name)+'&size='+encodeURIComponent(size)+'">' + span + '</a>';
                            }
                ...
                toolbar: function() {
                    if (settings.editButton || settings.deleteButton) {
                        ...
                        if ($table.find('th.tabledit-toolbar-column').length === 0) {
                            $table.find('tr:first').append('<th class="tabledit-toolbar-column">操作</th>');
                        }
                        ...
                        if (settings.storageButton) {
                            storageButton = '<button type="button" class="tabledit-storage-button ' + settings.buttons.storage.class + '" style="float: none;">' + settings.buttons.storage.html + '</button>';
                        }

                        var toolbar = '<div class="tabledit-toolbar ' + settings.toolbarClass + '" style="text-align: left;">\n\
                                           <div class="' + settings.groupClass + '" style="float: none;">' + editButton + storageButton + deleteButton + '</div>\n\
                                           ' + saveButton + '\n\
                                           ' + confirmButton + '\n\
                                           ' + restoreButton + '\n\
                                       </div></div>';
                        ...
                        $table.find('tbody>tr').append('<td class="canEdit" style="white-space: nowrap; width: 1%;">' + toolbar + '</td>');
                    }
                }
            }
        };
        ...
        var Mode = {
            view: function(td) {
                ...
                if (settings.editButton) {
                    ...
                    $tr.find('button.tabledit-storage-button').show();
                }
            },
            edit: function(td) {
                Delete.reset(td);
                ...
                if (settings.editButton) {
                    ...
                    $tr.find('button.tabledit-storage-button').hide();
                }
            }
        };
        ...
        if (settings.deleteButton) {
            ...
            $table.on('click', 'button.tabledit-confirm-button', function(event) {
                ...
                var td_index = $(this).parent().parent().parent().prop('id');
                
                $.post({
                    url: 'api/deleteMerchandise.php',
                    data: JSON.stringify({
                        // User
                        // account: "admin",
                        token: token,
                        merchandiseID: td_index           
                    }),      
        ...
        if (settings.editButton) {
            ...
            $table.on('click', 'button.tabledit-edit-button', function(event) {
                ...
                var flag = $(this).parent().parent().parent().parent().attr('id');
                for(var old_index=0;old_index<merchandiseData.data.merchandise.length;old_index++){  
                    if(merchandiseData.data.merchandise[old_index].ID == flag){
                        old_name = merchandiseData.data.merchandise[old_index].name;    
                        old_size =  merchandiseData.data.merchandise[old_index].size;
                        old_warning =  parseInt(merchandiseData.data.merchandise[old_index].warning);
                        old_price  = parseInt(merchandiseData.data.merchandise[old_index].price);
                        old_departmentID = parseInt(merchandiseData.data.merchandise[old_index].departmentID);
                    }
                }      
                $(this).parent().parent().parent().parent().find("select[name='department'] option[value="+ old_departmentID +"]").prop("selected",true);                  
            });
            ...
            $table.on('click', 'button.tabledit-save-button', function(event) {
                ...
                var new_name = $(this).parent().parent().parent().find("input[name='name']").val();
                var new_size = $(this).parent().parent().parent().find("input[name='size']").val();
                var new_department = parseInt($(this).parent().parent().parent().find("select[name='department']").find('option:selected').val());  
                var new_warning = parseInt($(this).parent().parent().parent().find("input[name='warning']").val());
                var new_price = parseInt($(this).parent().parent().parent().find("input[name='price']").val());
                var td_index = parseInt($(this).parent().parent().parent().prop('id'));
                ...
                $.post({
                    ...
                    data: JSON.stringify({
                        ...
                        old_price: old_price,
                        old_departmentID: old_departmentID
                    }),      
                    success: function (jsonData) { // Callback function when successful
                        viewDeletemessage(jsonData); // Call VIEW function 
                    },

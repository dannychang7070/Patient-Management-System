    ...
    $.fn.Tabledit = function(options) {
                ...
                toolbar: function() {
                    if (settings.editButton || settings.deleteButton) {
                        ...
                        if (settings.resetButton) {
                            resetButton = '<button type="button" class="tabledit-reset-button ' + settings.buttons.reset.class + '" style="float: none;">' + settings.buttons.reset.html + '</button>';
                        }

                        var toolbar = '<div class="tabledit-toolbar ' + settings.toolbarClass + '" style="text-align: left;">\n\
                                           <div class="' + settings.groupClass + '" style="float: none;">' + editButton + resetButton + deleteButton + '</div>\n\
                                           ' + saveButton + '\n\
                                           ' + confirmButton + '\n\
                                           ' + restoreButton + '\n\
                                       </div></div>';
        ...
        if (settings.deleteButton) {
            ...
            $table.on('click', 'button.tabledit-confirm-button', function(event) {
                ...
                var delete_ID = parseInt($(this).parent().parent().parent().attr("id"));
                $.post({
        ...
        if (settings.editButton) {
            ...
            $table.on('click', 'button.tabledit-save-button', function(event) {
                ...
                var edit_ID = parseInt($(this).parent().parent().parent().attr("id"));
                var user_Name = $(this).parent().parent().parent().find("td:nth-child(2)").text();
                var user_Account = $(this).parent().parent().parent().find("td:nth-child(2)").attr('class');
                var department_ID = parseInt($(this).parent().parent().parent().find("select[name='department']").find('option:selected').val());
                var permission_ID = parseInt($(this).parent().parent().parent().find("select[name='role']").find('option:selected').val());
                ...
                $.post({
                    ...
                    success: function (jsonData) { // Callback function when successful
                        viewSavemessage(jsonData); // Call VIEW function 
                    },
                    fail: function () {
                        alert("ERROR: Database connection failed.");
                    },
